...
public class GlobalHandlerException {
    ...
    public ResponseEntity<ApiResponse> handleInvalidArgException(MethodArgumentNotValidException e, WebRequest request) {
        ...
        ApiResponse response = new ApiResponse(
            ...
            fieldErrors.isEmpty() ? "Erro de Validação" : fieldErrors.get(0).getDefaultMessage(),
            request.getDescription(false)
        );
    ...
    public ResponseEntity<ApiResponse> handleMessageNotReadableException(HttpMessageNotReadableException e, WebRequest request) {
        ApiResponse response = new ApiResponse(
            ...
            "Requisição inválida: Formato de dado incorreto",
            request.getDescription(false)
        );
    ...
    public ResponseEntity<ApiResponse> handleIllegalArgumentException(IllegalArgumentException e, WebRequest request) {
        ApiResponse response = new ApiResponse(
            Instant.now(),
            HttpStatus.BAD_REQUEST.value(),
    ...
    public ResponseEntity<ApiResponse> handleDataIntegrityViolationException(DataIntegrityViolationException e, WebRequest request) {
        ApiResponse response = new ApiResponse(
            Instant.now(),
            HttpStatus.BAD_REQUEST.value(),
            HttpStatus.BAD_REQUEST.getReasonPhrase(),
